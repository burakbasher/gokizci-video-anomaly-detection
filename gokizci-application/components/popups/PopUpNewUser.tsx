"components/popups/PopUpNewUser.tsx"

"use client";

import React, { useState, ChangeEvent } from 'react';
import { BlackButton } from '../buttons/BlackButton';
import { userFormControls, Errors, initialErrors, initialFormValues } from '@/app/lib/definitions';
...
export function PopUpNewUser({ onClose }: PopUpNewUserProps) {
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            tempErrors.role = 'Hata, lütfen tekrar deneyin.';
            setErrors(tempErrors);
            console.log(error);
        }
    };
    ...
    return (
        <div className="overflow-y-auto overflow-x-hidden bg-background/40 fixed top-0 right-0 left-0 z-50 justify-center items-center w-full h-full flex">
            <div className="relative p-4 w-[380px] max-w-md max-h-full">
                <div className="relative border border-stroke-main rounded-lg bg-background-surface shadow-lg border-t-4 border-t-primary">
                    <button
                        type="button"
                        onClick={onClose}
                    ...
                    <form className="m-6 grid gap-4" onSubmit={handleSubmit}>
                        <p className="text-dark text-center text-2xl font-medium">Yeni Kullanıcı Ekle</p>
                        {userFormControls.map(control => (
                            <div key={control.id} className='relative'>
                                <div className="relative">
                                    {control.componentType === 'input' && control.type !== 'password' && (
                                        <input
                                            type={control.type}
                                            id={control.id}
                                            value={formValues[control.id as keyof typeof formValues]}
                                            onChange={handleInputChange}
                                            className={`block px-2.5 pb-2.5 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border ${errors[control.id as keyof typeof errors] ? 'border-red-600' : 'border-stroke-main'} appearance-none dark:text-white dark:border-stroke-main dark:focus:border-stroke-dark focus:outline-none focus:ring-0 focus:border-stroke-dark peer`}
                                            placeholder=" "
                                        />
                                    )}
                                    {control.componentType === 'input' && control.type === 'password' && (
